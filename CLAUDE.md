...
| `search_relevant` | ~200-400 | Find files/symbols by concept or partial name |
| `read_source` (symbol) | ~50-300 | Read just one function/struct |
| `get_token_estimate` | ~100 | Check cost before deciding to Read |
...
1. `search_relevant` — find files/symbols related to your task by concept, partial name, or type pattern. Searches across paths, names, signatures, and doc comments with ranked results. **Start here when you know what you're looking for but not where it is.**
2. `get_tree` — see directory/file layout. Use `path` param to scope to a subtree.
3. `get_file_summary` — get a complete overview of any file (declarations, imports, counts) WITHOUT reading it.
...
6. `search_signatures` — find functions/methods by signature substring (e.g., `"-> Result<"`, `"&mut self"`).
7. `get_token_estimate` — before deciding to `Read` a file, check how many tokens it costs. Compares full-file cost vs read_source cost and gives a recommendation.
8. `read_source` — read source code by **symbol name** or explicit line range. Cap: 200 lines. Use `expand` param for context lines.
9. `list_declarations` — list all declarations in a file. Use `kind` filter (fn, struct, class, trait, enum, etc.) and `shallow` mode to reduce output.
10. `get_imports` — get import statements for a file.
11. `get_stats` — codebase stats: file count, line count, language breakdown, indexing duration.
12. `regenerate_index` — re-index after code changes. Updates INDEX.md and refreshes the in-memory index.
...
- `src/mcp.rs` — MCP server + all 12 tool implementations
